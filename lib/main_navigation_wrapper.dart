...
class _CartScreenState extends State<CartScreen> {
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              Icon(_getPaymentIcon(state.selectedPayment), color: state.selectedPayment.contains('Cash') ? Colors.green : AppTheme.secondary),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
  ...
  IconData _getPaymentIcon(String method) {
    if (method.contains('Cash')) return Icons.payments;
    if (method.contains('Wallet')) return Icons.account_balance_wallet;
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _buildPaymentOption(state, 'UPI: avi@okaxis', Icons.payment, AppTheme.secondary),
            _buildPaymentOption(state, 'Visa ending in 4242', Icons.credit_card, AppTheme.secondary),
            _buildPaymentOption(state, 'Cash on Delivery (COD)', Icons.payments, Colors.green),
            _buildPaymentOption(state, 'HomePlate Wallet (₹${state.walletBalance.round()})', Icons.account_balance_wallet, AppTheme.primary),
            const SizedBox(height: 24),
          ],
        ),
      ),
  ...
  Widget _buildPaymentOption(AppState state, String method, IconData icon, Color color) {
  ...
  void _placeOrder(BuildContext context, AppState state, double grandTotal) async {
    try {
      ...
      showDialog(
        ...
        builder: (context) => _buildSuccessUI(context, grandTotal, state.selectedPayment),
      );
  ...
  Widget _buildSuccessUI(BuildContext context, double total, String method) {
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('₹${total.round()} confirmed via $method', style: const TextStyle(color: AppTheme.textMuted)),
